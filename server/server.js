...
const { resource_types } = require("cloudinary/lib/api");
// const { request } = require("express");
// const { default: user } = require("../client/src/hoc/user");
// const { request } = require("express");

//========== PRODUCTS ===========

//post because is listening for json data
app.post("/api/product/shop", (req, res) => {
  ...
  for (let key in req.body.filters) {
    if (req.body.filters[key].length > 0) {
      if (key === "price") {
        findArgs[key] = {
          $gte: req.body.filters[key][0], //firts element in array 0 300 600 etc
          $lte: req.body.filters[key][1], //second element in array 299, 599 etc
        };
      } else {
...
app.post("/api/users/addToCart", auth, (req, res) => {
  User.findOne({ _id: req.user._id }, (err, doc) => {
    let duplicate = false;
    //verific daca este un produs asemanator in cosul de cumparaturi
